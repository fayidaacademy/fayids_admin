...
export default function UploadPrizeImage(params: any) {
  // const PrizeId = params.prize_id;
  const prizeId = params.prizeId;
  //console.log("BlogIdrrrr: " + prizeId);
  ...
  //   // Set up the request
  //   xhr.open("POST", `${apiUrl}/prizes/upload_prize_images/${prizeId}`);
  //   xhr.send(formData);
  // };
  ...
  const handleSubmit = async (event: any) => {
    ...
    try {
      ...
      const response = await fetch(
        `${apiUrl}/prizes/upload_prize_images/${prizeId}`,
        {
          method: "POST",
          body: formData,
        }
 ...
 <div>{uploading ? <h1>Uploading In Progress</h1> : <h1></h1>}</div>
      <form
        method="POST"
        action={`${apiUrl}/prizes/upload_prize_images/${prizeId}`}
        id="myForm"
