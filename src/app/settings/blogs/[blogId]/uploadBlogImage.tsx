...
export default function UploadBlogImage(params: any) {
  ...
  //   // Set up the request
  //   xhr.open("POST", `${apiUrl}/blogs/upload_blog_images/${BlogId}`);
  //   xhr.send(formData);
  // };
  ...
  const handleSubmit = async (event: any) => {
    ...
    try {
      ...
      const response = await fetch(
        `${apiUrl}/blogs/upload_blog_images/${BlogId}`,
        {
          method: "POST",
          body: formData,
        }
  ...
  return (
    <div>
      ...
      >
        ...
        >
          <span className="text-blue-900 font-semibold">Update Blog Image</span>
        </label>
        <input
          type="file"
          className="hidden"
