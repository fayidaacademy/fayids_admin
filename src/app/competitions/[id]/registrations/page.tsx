...
export default function CompetitionRegistrations({ params }: { params: { id: string } }) {
  ...
  const fetchRegistrations = async () => {
    try {
      ...
      const compResponse = await fetch(`${apiUrl}/admin/competitions/${params.id}`, {
        headers: {
      ...
      // Fetch registrations
      const response = await fetch(`${apiUrl}/admin/competitions/${params.id}/registrations`, {
        headers: {
          "Authorization": `Bearer ${token}`,
          "Content-Type": "application/json",
        },
  ...
  const handleExport = () => {
    ...
    const headers = ["Exam ID", "Name", "Email", "Phone", "Grade", "School", "Region", "City", "Submissions", "Registered At"];
  ...
  const stats = {
    ...
    averageSubmissions: registrations.length > 0 
      ? (registrations.reduce((sum, r) => sum + r.submissionsCount, 0) / registrations.length).toFixed(1)
      : 0
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <Card className="glass-card border-0 shadow-lg">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-600 mb-1">With Submissions</p>
                <p className="text-2xl font-bold text-gray-900">{stats.withSubmissions}</p>
              </div>
        ...
        <Card className="glass-card border-0 shadow-lg">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-600 mb-1">Avg Submissions</p>
                <p className="text-2xl font-bold text-gray-900">{stats.averageSubmissions}</p>
              </div>
      ...
      <Card className="glass-card border-0 shadow-lg">
        <CardHeader>
          ...
          <CardDescription>Students who registered for this competition</CardDescription>
        </CardHeader>
        <CardContent>
          {loading ? (
                      ...
                      <div className="flex-1">
                        <div className="flex items-center space-x-3 mb-3">
                          <div className="flex items-center justify-center w-12 h-12 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 text-white font-semibold">
                            {registration.student.firstName[0]}{registration.student.lastName[0]}
                          </div>
                          <div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-3">
                          <div className="flex items-center space-x-2 text-sm">
                            ...
                            <span className="text-gray-900">{registration.student.email}</span>
                          </div>

                          <div className="flex items-center space-x-2 text-sm">
                            <Phone className="h-4 w-4 text-gray-400" />
                            <span className="text-gray-900">{registration.student.phoneNumber}</span>
                          </div>

                          <div className="flex items-center space-x-2 text-sm">
                            <School className="h-4 w-4 text-gray-400" />
                            <span className="text-gray-900">{registration.student.schoolName}</span>
                          </div>

                          <div className="text-sm">
                            <span className="text-gray-600">Region: </span>
                            <span className="text-gray-900">{registration.student.region}</span>
                          </div>

                          <div className="text-sm">
                            <span className="text-gray-600">City: </span>
                            <span className="text-gray-900">{registration.student.city}</span>
                          </div>
                        ...
                        {registration.submissions && registration.submissions.length > 0 ? (
                          <div className="mt-4 pt-4 border-t border-gray-200">
                            <p className="text-sm font-medium text-gray-600 mb-2">
                              Submissions ({registration.submissionsCount}) • Avg Score: {calculateAverageScore(registration.submissions)}
                            </p>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                              {registration.submissions.map((submission) => (
                                <div key={submission.id} className="p-3 rounded bg-white border border-gray-200">
